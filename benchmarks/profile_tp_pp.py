...
def request_once(url: str, model: str, prompt: str, max_tokens: int) -> tuple[float, int]:
    payload = json.dumps(
        {
            "model": model,
...
def main() -> None:
    ...
    for _ in range(args.warmup):
        request_once(endpoint, args.model, args.prompt, args.max_tokens)
    ...
    output_units = sum(length for _, length in results)
    summary = {
        ...
        "ttft_ms": None,
        "note": "output_units is response character count; TTFT requires streaming API.",
    }
